...
def rotate_velocities_with_PCA(ds_velo):
        ...
        print(f"Degrees of components {m.degrees(np.arctan2(components[0,1], components[0,0])):.0f}, {m.degrees(np.arctan2(components[1,1], components[1,0])):.0f}")
...
# time vector
    ...
    for ii in np.arange(ns, dtype='int'):
        time.append(datetime(year=int(mat_file['SerYear'][ii]+2000),month=mat_file['SerMon'][ii][0],day=mat_file['SerDay'][ii][0],hour=mat_file['SerHour'][ii][0],minute=mat_file['SerMin'][ii][0],second=mat_file['SerSec'][ii][0]))
...
# construct vertical axis
    mdepth = (gsw.z_from_p(p=np.mean(mat_file['AnDepthmm'][500:-500].squeeze()) * 1e-3, lat=64) *-1).round(0) # mean of pressure sensor record
# alternatively, you can use water depth (if known) and mooring sketch
    print(f"Mounting depth: {int(mdepth)}")
    zax = mdepth - mat_file['RDIBin1Mid'].squeeze() - np.concatenate(([0], np.cumsum(mat_file['RDIBinSize'] * np.ones(nb-1)))).squeeze()  # z = water depth
...
# make xarray dataset with coords and dims time and zax
    ds_velo = xr.Dataset(coords = {'time':np.array(time), 'z':zax}, attrs = {'units':'mm/s',})
    for i in range(len(data)):
        da = xr.DataArray(mat_file[data[i]], coords = {'time':np.array(time), 'z':zax},
                                    attrs  = {
                        '_FillValue': -32768,
                        'units'     : 'mm/s',},
                        name = data[i])
    ...
    for i in range(len(data_time)):
        da = xr.DataArray(mat_file[data_time[i]].squeeze(), coords = {'time':np.array(time)},
                        name = data_time[i])
...
# find and remove outliers: where velocity is more than 3 standard deviations from the rolling median
    ...
    ds_velo = ds_velo.where(np.abs(ds_velo.SerMagmmpersec - 
                         ds_velo.SerMagmmpersec.rolling(time=int(n_obs_day/24*25), center=True, min_periods=int(0.7*n_obs_day)).median())
                         < std_dev,
                           other= np.nan)
...
# make all values in flag 0 where ds_velo.Magmmpersec is nan
    ds_velo_no_outlier_removal['flag'] = ds_velo_no_outlier_removal.flag.where(~np.isnan(ds_velo_no_outlier_removal.SerMagmmpersec), 1)
    ...
    ds_velo['Alongmmpersec'] = xr.DataArray(speed_along.transpose().reshape(ns,len(zax)), dims=['time', 'z'], attrs={'units': 'mm/s'})
    ds_velo['Acrossmmpersec'] = xr.DataArray(speed_across.transpose().reshape(ns,len(zax)), dims=['time', 'z'],     attrs={'units': 'mm/s'})
...
# calculate tides and remove
    ...
    df_const = pd.DataFrame(my_tide_across.model, index=constituent).drop('constituent', axis=1).sort_values(by='amplitude', ascending=False).rename(columns={'amplitude': 'Velocity amplitude [mm/s]', 'phase': 'Phase [deg]'})
    ...
    return ds_velo, constituent, df_const, my_tide_along, my_tide_across, my_prediction_along, my_prediction_across, mean_velocity_water_column_along, mean_velocity_water_column_across, ds_velo_no_outlier_removal

if __name__ == "__main__":
   ds_velo, constituent, df_const, my_tide_along, my_tide_across, my_prediction_along, my_prediction_across, mean_velocity_water_column_along, mean_velocity_water_column_across, ds_velo_no_outlier_removal = open_and_proces_mat_adcp(f300)
